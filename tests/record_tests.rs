...
fn record_set_get_remove_and_parent_reference() {
    ...
    assert_eq!(record.object("name"), Some(&RecordValue::String("hello".into())));
    assert_eq!(record.parent().map(|reference| reference.record_id().record_name()), Some(parent_id.record_name()));

...
fn subscriptions_build_into_any_subscription() {
    ...
    let zone = CKRecordZoneSubscription::new(CKRecordZoneID::new("Tasks", "__defaultOwner__"), "zone-sub")
        .with_notification_info(notification.clone());
    let database = CKDatabaseSubscription::new("db-sub").with_notification_info(notification);

    let subscriptions = [CKAnySubscription::from(query), CKAnySubscription::from(zone), CKAnySubscription::from(database)];
